...
**Routing (`src/App.tsx`):** routes map 1:1 to `src/features/<name>/<Name>Page.tsx`. The `<Route>`s are children of `PageSwiper` (`src/components/PageSwiper.tsx`), which renders them in one of two slots so a horizontal swipe can mount the neighbouring tab *for real* while dragging and slide both pages as one strip (order = `navItems` in `BottomNav.tsx`). Positions are set imperatively on the slot elements — don't put per-frame gesture state in React state. `ProgressionPage` and `BodyweightPage` are loaded via `React.lazy` because they pull in `recharts` (~350 kB); Vite splits this into a shared `chart` chunk. Follow the same lazy pattern for any new chart-heavy page rather than importing `recharts` at the top level.
