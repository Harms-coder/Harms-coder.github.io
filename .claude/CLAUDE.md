...
**Routing (`src/App.tsx`):** routes map 1:1 to `src/features/<name>/<Name>Page.tsx`. The `<Route>`s are children of `PageSwiper` (`src/components/PageSwiper.tsx`), which renders them in one of two slots so a horizontal swipe can mount the neighbouring tab *for real* while dragging and slide both pages as one strip (order = `navItems` in `BottomNav.tsx`). The neighbouring tabs are pre-mounted (hidden, off-screen) ~450 ms after landing on a page and dropped again whenever a readwrite IndexedDB transaction completes (`DB_CHANGED_EVENT` in `src/db/database.ts`, emitted by a patched `IDBDatabase.prototype.transaction`) — so a tab page must never navigate or write on mount, only on user action, or it would fire while hidden. Every `<Routes>` gets a `location` prop (adding/removing it remounts the page). Positions are set imperatively on the slot elements — don't put per-frame gesture state in React state. Scroll position per tab is remembered here too (the app-level scroll reset was removed). `ProgressionPage` and `BodyweightPage` are loaded via `React.lazy` because they pull in `recharts` (~350 kB); Vite splits this into a shared `chart` chunk. Follow the same lazy pattern for any new chart-heavy page rather than importing `recharts` at the top level.

**Design system:** single fixed theme (dark, premium, bronze/orange accent — not adaptive to OS light mode). Colors are CSS custom properties defined under `@theme` in `src/index.css` (`--color-bg`, `--color-surface`/`-2`/`-3`, `--color-accent`/`-bright`/`-glow`/`-dark`, `--color-text`/`-secondary`/`-muted`, `--color-success`, `--color-danger`, etc.), referenced from JSX as Tailwind arbitrary-value classes, e.g. `bg-(--color-surface)`, `text-(--color-text-muted)`. Change the palette by editing those tokens only — never hardcode a hex color in a component. Two shared utility classes carry the rest of the visual system and are applied via `className` rather than duplicated inline: `.card-shadow` (the drop shadow every card/tile uses) and `.accent-fill` (the bronze gradient + glow used on every primary button and every "active" pill/tab/segmented-control state). When adding a new card or active-state control, reuse these classes instead of hand-rolling the shadow/gradient again. All icons are hand-drawn inline SVGs in `src/components/icons.tsx` — no emoji and no icon library anywhere in the UI; add new icons there in the same style (24×24 viewBox, `stroke="currentColor"`, head/weight circles filled `currentColor` for a bolder look). `ExerciseIcon` (`src/components/ExerciseIcon.tsx`) picks a movement-specific pictogram via `src/lib/exerciseIcon.ts` (keyword + category matching over `getExerciseIcon`) rather than one generic glyph — see that file before adding new exercises whose movement doesn't already map cleanly. The weight-comparison pictures on the Overview ("Det svarer til ca. 4 elefanter") come from `src/assets/comparisons/<kind>.png` (320×320 transparent PNG; prepare new ones with `python3 scripts/fit-image.py in.png out.png`, kinds are the `ComparisonKind` union in `src/lib/weightComparisons.ts`). `comparisonPhotos.ts` finds them via `import.meta.glob`, so dropping a file in is enough — kinds with a picture are preferred by `getWeightComparison`, the rest show a drawn silhouette. The silhouettes are generated: edit the shapes in `scripts/silhouettes.py` and run `python3 scripts/silhouettes.py`, which rewrites `src/features/overview/comparisonSilhouettes.ts` and a preview `scripts/silhouettes.html` — never hand-edit the generated path strings. `CardActions` (`src/components/CardActions.tsx`) is the shared 3-dot edit/delete menu used by every entry card (`ExerciseCard`, `RoutineCard`, `CardioEntryCard`, `BodyweightEntryCard`) instead of two standalone icon buttons — reuse it for any new editable/deletable card rather than re-adding bare edit/delete buttons.
...
**Hosting / deploy:** the app is live at https://harms-coder.github.io/ (public repo `Harms-coder/Harms-coder.github.io`, GitHub Pages with the Actions source). Every push to `main` runs `.github/workflows/deploy.yml` (`npm ci && npm run build` → Pages); it's live ~1 min later and installed apps pick it up on next launch via the service worker (`vite-plugin-pwa`, `registerType: 'autoUpdate'`, configured in `vite.config.ts` — app shell precached, images/fonts cached on use). `npm run build` also copies `dist/index.html` to `dist/404.html` so deep links work on Pages. The old LAN dev-server install (`http://<mac-ip>:5173`) is a different origin with its own IndexedDB — migrate via backup/restore, don't expect data to carry over.

**PWA install assets:** `public/manifest.json` + `public/icon-180/192/512.png` (cut from the Vigorra brand sheet — leaf-textured triangle mark on a dark leaf square; `public/icon.svg` is the old dumbbell mark and no longer the source) power "Add to Home Screen" on iOS/Android — see `index.html`'s `apple-touch-icon`/`manifest` links. This makes the home-screen launch a standalone window with its own storage, separate from Safari or any in-app browser. The in-app logo mark is `src/assets/logo-mark.png` (same mark, cream keyed out to transparent), used by `VigorraLogo`. Font is Jost (Google Fonts, linked in `index.html`).
