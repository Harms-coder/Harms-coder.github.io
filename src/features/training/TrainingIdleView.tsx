    ...
    <div className="flex flex-col gap-4 px-4 pt-6 text-left">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-(--color-border) bg-(--color-surface) p-4 card-shadow">
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[16px] font-semibold text-(--color-text)">Dagens plan</span>
          </div>
          {plan && (
        ...
        {plan ? (
          <Link to="/kalender" className="flex flex-col gap-2 active:opacity-70">
            <span className="text-[15px] font-medium text-(--color-text)">{plan.title}</span>
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              <StatItem icon={IconList} text={`${plan.exerciseCount} øvelser`} />
              <StatItem
                icon={IconClock}
                text={`${plan.exerciseCount * MIN_PER_EXERCISE_LOW}-${plan.exerciseCount * MIN_PER_EXERCISE_HIGH} min`}
              />
              {plan.categoryCount > 0 && (
                <StatItem icon={IconActivity} text={`${plan.categoryCount} muskelgrupper`} />
              )}
            </div>
            {plan.categoriesLine && (
      ...
      {lastSession && (
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="text-[16px] font-semibold text-(--color-text)">Sidste træning</span>
          </div>
          <span className="text-[15px] font-medium text-(--color-text)">{lastSession.title}</span>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
            {lastSession.durationMin !== undefined && (
              <StatItem icon={IconClock} text={`${lastSession.durationMin} min`} />
            )}
            <StatItem icon={IconClipboard} text={`${lastSession.setCount} sæt`} />
            <StatItem icon={IconCalendar} text={lastSession.dateLabel} />
          </div>
        </Link>
      )}
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-(--color-border) bg-(--color-surface) p-4 card-shadow">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[16px] font-semibold text-(--color-text)">Hurtigt overblik</span>
        </div>
        <div className="flex items-stretch justify-between">
          ...
          <div className="flex flex-1 flex-col items-center gap-1">
            ...
            <span className="text-[15px] font-semibold text-(--color-text)">{streakDays} dage</span>
          </div>
          <div className="w-px flex-shrink-0 bg-(--color-border)" />
          <div className="flex flex-1 flex-col items-center gap-1">
            <span className="flex items-center gap-1.5 text-[12px] text-(--color-text-muted)">
      ...
      <div className="flex items-center gap-3 rounded-2xl border border-(--color-border) bg-(--color-surface) p-4 card-shadow">
        ...
        <div className="flex flex-col gap-0.5">
          <span className="text-[16px] font-semibold text-(--color-text)">Tip</span>
          <span className="text-[13px] text-(--color-text-muted)">
